...
**Important:** `MlKitOcrAnalyzer` wraps a Task-based ML Kit API behind a synchronous `OcrAnalyzer` contract. `TextRecognizer.process(...)` must not be treated as if `getResult()` plus a null check were a valid readiness test. The OCR task must be awaited explicitly on the OCR worker thread (e.g. via `Tasks.await(...)`), and failure or interruption must resolve to `OcrResult.empty()` rather than crashing the app.
