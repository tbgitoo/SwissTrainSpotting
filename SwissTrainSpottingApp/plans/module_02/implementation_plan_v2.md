...
### Create
- `app/src/main/java/com/tb/swisstrainspotting/ui/AcquisitionMode.java`
...
Implement Module 2 Phase D automated tests only: add instrumentation tests for drawable presence, 90° EXIF-corrected image display, and activity recreation persistence using repository-controlled fixtures `Landscape_1.jpg` and `Landscape_6.jpg` from Android test assets. Prefer deterministic in-app URI/result injection over automating the external picker UI. Keep manual full-flow sanity checks outside this prompt.
